...
def Hamiltonian(kx, ky, U=0.0, Delta=0.8):
    ...
    H[0,1] = ta + tb*np.exp(-1j*kx)                         # Aup   from Bup
    H[0,2] = tp * (1 + np.exp(-1j*kx))                      # Aup   from Cup
    H[0,3] = tq * (1 + np.exp(-1j*kx))                      # Aup   from Dup

    H[1,2] = tq * (1 + np.exp(1j*ky))                       # A2up   from B1up
    H[1,3] = tp * (1 + np.exp(1j*kx))                       # A2up   from B2up

    H[2,3] = (ta*np.exp(1j*kx) + tb)*np.exp(-1j*ky)         # B1up   from B2up
...
def Current(kx, ky, mu):
    ...
    if (mu == "x"):
        ...
        J[0,1] =-1j * tb * np.exp(-1j*kx)
        J[0,2] =-1j * tp * np.exp(-1j*kx)

        J[1,3] = 1j * tp * np.exp(1j*kx)

        J[2,3] = 1j * ta * tb*np.exp(1j*(kx-ky))
        ...
        J[0,3] =-1j * tq * np.exp(-1j*ky)

        J[1,2] = 1j * tq * np.exp(1j*ky)

        J[2,3] =-1j * (ta*np.exp(1j*kx) + tb) * np.exp(-1j*ky)
...
def SpinCurrent(kx, ky, mu):
    ...
    if (mu == "x"):
        ...
        J[4,6] =-J[0,2]                                         # A1down from B2down
